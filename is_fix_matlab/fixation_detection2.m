...
% modify and adapted from fixation_detection.m 
...
% chengchi, March 6, 2018

function [t_ind]=fixation_detection2(data,t1,t2,minDur,s)
%load data from file
% data=load(data);
x=data(:,1);
y=data(:,2);
...
%final output
fixation_list_t2=fixation_list_t2(:,5:7);

% 
t_ind = {};
for f = 1:size(fixation_list_t2,1)
    fix1 = fixation_list_t2(f,:);
    s_ind = find(t == fix1(1));
    e_ind = find(t == fix1(2));
    
    if ( numel(s_ind) > numel(e_ind) )
      s_ind = s_ind(1:numel(e_ind));
    end
    
    t_ind{f} = [s_ind e_ind]+s-1;
end
